...
function exit_time(sys::StochSystem, x_i::State, x_f::State;
    ...
    trans = transition(sys, x_i, x_f; rad_i, rad_f, dt, tmax, solver, progress, rad_dims, kwargs...)[2:3]; # run the transition function with cut_start=true
...
function exit_times(trans::Tuple;
    cut_start::Bool=true,
    x_i::State=[],
	...
	ets = zeros(length(trans[1]),2);
    for ii ∈ 1:length(trans[1])
        if cut_start
            ets[ii,:] = [trans[2][ii][1], trans[2][ii][end]-trans[2][ii][1]];
        elseif ~correlated
			sim = trans[1][ii]; simt = trans[2][ii];
		    idx = size(sim,2);
            ...
            while dist > rad_i
     		...
     		ets[ii,:] = [simt[idx], simt[end] - simt[end]-simt[idx]]; 
        else # i.e. cut_start = false and correlated = true
            sim = trans[1][ii]; simt = trans[2][ii];
            idx = size(sim)[2]
            ...
            ets[ii,:] = [simt[idx], simt[end]-simt[idx]]; 
        end
    end
    ets, trans[3], trans[4]
...
function residence_times2(sys::StochSystem, x_i::State, x_f1::State, x_f2::State, N=1;
    ...
    times::Vector, idx::Array{Float64}, fails = zeros(Float64,N), zeros(Float64, N, 2), Threads.Atomic{Int}(0)
