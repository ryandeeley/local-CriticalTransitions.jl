...
across the range of physical time points z in the discretisation, say  ``z \\in [k(\\Delta z): k \\in \\{0, 1, 2, ..., N\\}]``. In line with the boundary conditions of the Langevin MCMC SPDE problem, this partial derivative has value zero at the physical start and end times ``z\\in \\{0,T_{\\text{phys}}\\}``. Here ``\\boldsymbol{b}, \\sigma`` and ``\\Sigma`` are the drift field, noise strength and covariance matrix of `sys`, respectively.  

## Function arguments: 
* `u`: the transition path at virtual time t. Given that the number of discrete points in path space is ``N+1``, and `sys` is ``d``-dimensional, one should enter a concatenated vector of length ``M:=(N+1)d`` of the form ``[x_1(t,0), x_1(t,\\Delta z), ..., x_1(t, N\\Delta z), ....., x_d(t,0), x_d(t,\\Delta z), ..., x_d(t, N\\Delta z)]``.
...
The function returns an ``M``-dimensional vector of partial-deriviates ``\\bigg[\\frac{\\partial x_1}{\\partial t}(t,0), \\frac{\\partial x_1}{\\partial t}(t,\\Delta z), ..., \\frac{\\partial x_1}{\\partial t}(t,N\\Delta z), ....., \\frac{\\partial x_d}{\\partial t}(t,0), \\frac{\\partial x_d}{\\partial t}(t,\\Delta z), ..., \\frac{\\partial x_d}{\\partial t}(t,N\\Delta z)\\bigg]``. 
"""
...
function langevinmcmc_mf1(sys::StochSystem, init;    
    T = 15,         # physical end time
    Δt = 1e-3,      # virtual timestep
    tmax = 250.0,   # virtual end time  
    savestep=10     # number of steps after which another path is saved
    )

    N = size(init, 2) # number of phys time steps in initial cond. gives number of phys time steps of the path 
    t = range(0, T; length = N); # the physical time range
    Δz = T/(N-1)    # the physical time step

    Nstep = ceil(Int64, tmax/Δt); # the number of mcmc iterations

    paths = zeros(1+floor(Int64,Nstep/savestep), length(sys.u), N); # three-dimensional array to store the paths
    paths[1,:,:] = init; # storing the initial condition
    t = 0; # the current virtual time value 
    pathstemp = copy(init); # storing the initial condition
    ...
    savecounter = 2
    println("----- Beginning sampling procedure -----")
    ...
    for it ∈ tqdm(1:Nstep)
        ...
        if 0==mod(it,savestep)
            #println("----- Saving path number $(savecounter) -----")
            #paths[savecounter,1,:] .= pathstemp[1,:]
            #paths[savecounter,2,:] .= pathstemp[2,:]
            paths[savecounter,:,:] = pathstemp
            savecounter += 1
        end
    ...
    paths #return paths[1:savecounter-1,:,:]

end

function langevinmcmc_mf2(sys::StochSystem, init;    
    T = 15,         # physical end time
    Δt = 1e-3,      # virtual timestep
    tmax = 250.0,   # virtual end time  
    savestep=10     # number of steps after which another path is saved
...
function stochastic_bridge(sys::StochSystem, Tphys::Float64, Δz::Float64) 
    ...
    u = zeros(length(sys.u)*N); # the dim-field of StochSystem
    ...
    g(u,p,t) = SVector{length(u)}(z); # defining the g-field of StochSystem

    pg = [];
    Σ = I(length(u));
    process = WienerProcess(0.,u);

    StochSystem(f, p, u, σ, g, pg, Σ, process)
