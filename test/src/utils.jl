...
"""
    is_multiplicative(sys::StochSystem)
Asserts whether a StochSystem has a multiplicative noise structure (true) or not (false).

> Warning: This function simply checks if there is a `x` in the symbolic output of the `sys.g` function.
"""
function is_multiplicative(sys::StochSystem)
    state_vars = @eval @variables $([Symbol("x$i") for i=1:length(sys.u)]...); # symbolic form of the state variables
    gout = sys.g(state_vars,sys.pg, @variables t); # symbolic output of the noise function
    occursin("x",String(Symbol(gout)))
end

"""
    is_multiplicative(sys::StochSystem)
Asserts whether a StochSystem is autonomous (true) or non-autonomous (false).

> Warning: This function simply checks if there is a `t` in the symbolic output of the `sys.f` function or `sys.g` function.
"""
function is_autonomous(sys::StochSystem)
    state_vars = @eval @variables $([Symbol("x$i") for i=1:length(sys.u)]...); # symbolic form of the state variables
    fout = sys.f(state_vars,p(sys),@variables t); # symbolic output of the drift function
    gout = sys.g(state_vars,p(sys),@variables t); # symbolic output of the noise function
    ~occursin("t",String(Symbol(fout))) && ~occursin("t",String(Symbol(gout))) 
end
