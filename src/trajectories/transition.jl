...
function transition_mf(sys::StochSystem, x_i::State, x_f::State;
    ...
    sim = simulate(sys, u0; dt, tmax, solver, callback=cb_exit, progress, save_everystep = false, save_start = false, kwargs...)
    ...
    while T < tmax && (reentered||exit)
        ...
        counter += 1; 
        if reentered
            sim = simulate(sys, u0; dt, tmax = tmax-T, solver, callback=cb_exit, progress, save_everystep = false, save_start = false, kwargs...); # overwrite the old solution
            T += sim.t[end]; # the new overall elapsed time
            u_in, u_out = [sim.u[end-1],sim.u[end]]; # where the run terminated, which becomes the new initial condition
            exit = true; reentered = false;  
        else #i.e. exit must be true 
...
function transition_mf(sysfunc::Function, process_init::State, x_i::State, x_f::State;
    rad_i=0.01,
    rad_f=0.01,
    dt=0.01,
    tmax=1e3,
    ...
    solver=EM(),
    progress=true,
    rad_dims=1:length(sysfunc(process_init).u),
    dispfreq::Int64 = 1000000,
    kwargs...)
...
function transitions_mf(sysfunc::Function, process_init::State, x_i::State, x_f::State, N=1;
    rad_i=0.01,
    rad_f=0.01,
    tmax=1.e3,
    dt=0.01,
