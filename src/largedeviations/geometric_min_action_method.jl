...
function geometric_min_action_method(sys::StochSystem, x_i::State, x_f::State, arclength=1.0;
    ...
    for i in 1:maxiter
        if showprogress
            println("\r... Iteration $(i), action value: $(S(path))")
        end

        if method == "HeymannVandenEijnden" 
            @warn("HeymannVandenEijnden method only implemented for non-multiplicative noise functions.")
            update_path = heymann_vandeneijnden_step(sys, path, N, arclength;
                tau, cov_inv=inv(sys.Σ))
        else
        ...
        for j in 2:N
            ...
            s[j] = s[j-1] + norm(update_path[:,j] - update_path[:,j-1]) 
        end # update_path is the new iterate of the path
        s_length = s/s[end]*arclength; # the normalised cumulative path lengths
        interp = ParametricSpline(s_length, update_path, k=3);
        path = reduce(hcat, [interp(x) for x in range(0, arclength, length=N)]); # reinterpolated path such that we have uniform spacing along arc length

        if simulated_annealing
            path[:,2:end-1] += σ*randn(length(sys.u),N-2)
            σ *= 1 - 0.01*r; # expontential decrease of the noise strength
        end
