...
function min_action_method(sys::StochSystem, init::Matrix, T::Real;
    ...
    f(x) = action(sys, fix_ends(x, init[:,1], init[:,end]),
        range(0.0, T, length=size(init, 2)), functional; kwargs...)
    
    #   f(x) = action(sys, fix_ends(x, init[:,1], init[:,end]),
    #    range(0.0, T, length=size(init, 2)), functional; cov_inv=cov_inv_along_path(sys, fix_ends(x, init[:,1], init[:,end]), sys.pg, 0.))
