...
See also [`modifiedtruscottbrindley_OU!`](@ref).
"""
function modifiedtruscottbrindley_OUρ(u,p,t)
    P, Z, X₁, X₂ = u
    α, β, γ, P₁, Z₁, ξ, ρ = p[1]
    ...
    dP, dZ = [f(P,Z); g(P,Z)] + [P/ξ 0; 0 Z]*Aρ*[X₁; X₂];
...
function modtbOU_αξγμσρ(α, ξ, γ, μ, σ, ρ; σₙ = 1., u0 = [0.,0.], stat_dist = false) # a convenient five-parameter version of the modifiedtruscottbrindley system 
    f(u,p,t) = modifiedtruscottbrindley_OUρ(u,p,t);
    β = 5/112; γ1 = 112/(45*0.0525); P₁ = β; Z₁ = 5/6; # standard parameters without α (growth rate) and ξ (time-scale separation)
    pf_wo_αξρ = [β, γ1, P₁, Z₁]; # parameters vector without α or ξ
    u = zeros(4);
    g(u,p,t) = [0 0; 0 0; 1 0; 0 1];
    pg = nothing; 
    Σ = [1 0; 0 1];
    ...
    StochSystem(f, vcat([α], pf_wo_αξρ, [ξ, ρ]), u, 1., g, pg, Σ, process)
end;